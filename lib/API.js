...
API.prototype.run = function (url, body, runInfo, callback) {
	...
	var that = this,
		endpoint = this._getEndpoint(url)
...
API.prototype._runRequest = function (req, res, callback) {
	...
	var endpoint = this._getEndpoint(req.url),
		that = this

	if (!endpoint) {
		// If endpoint is not found, we parse the body with component's
		// default parser for logging pourposes
		that._bodyParser(req, res, callback)
		return process.nextTick(function () {
	...
	endpoint.action.module.bodyParser(req, res, function (err) {
		if (err || !req.body) {
			return callback(err)
		}
 ...
 * @private
 */
API.prototype._getEndpoint = function (url) {
	// Translate 'v-last'

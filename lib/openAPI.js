...
module.exports = function (api, version) {
	...
	if (api._options.validate.defineTypes) {
		...
		Object.keys(typedefs).forEach(name => {
			spec.definitions[name] = typedefs[name].toJSONSchema('#/definitions')
		})
	}
	// Collect endpoints
	api.endpoints.forEach(endpoint => {
		...
		if (!isRest) {
			...
			if (inSchema) {
				pathItem.post.parameters[0].schema = inSchema.toJSONSchema('#/definitions')
			}
			if (outSchema) {
				pathItem.post.responses['200'].schema = outSchema.toJSONSchema('#/definitions')
			}
			...
			let parts = endpoint.name.split('/'),
				...
				url,
				successCode,
				schema = inSchema ? inSchema.toJSONSchema('#/definitions') : undefined,
			...
			// Add security info
			if (hasApiKeyAuth) {
				spec.paths[url][method].security = [{
					apiKey: []
				}]
			}
			if (hasOAuth) {
			...
			// Success responses
			if (outSchema) {
				let outSchemaJSON = outSchema.toJSONSchema('#/definitions')
				// Add schema description to schema according to contract
		...
		function makeErrorsTable(url, method, obj) {
			Object.keys(obj).forEach(statusCode => {
				let errorDescription = 'Payload object with [code, message]: \n'
